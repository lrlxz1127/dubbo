...
public class ReferenceConfig<T> extends AbstractReferenceConfig {
	...
	private void init() {
		...
		// 获取p2p调用url（直连url）
		// System.getProperty(interfaceName) >>>
		// System.getProperty("dubbo.resolve.file") >>>
		// new File(System.getProperty("user.home")), "dubbo-resolve.properties" >>>
		// properties.load(fis) >>>
		// dubbo-resolve.properties >>>
		// properties.getProperty(interfaceName)
	...
	private T createProxy(Map<String, String> map) {
		...
		if (c && !invoker.isAvailable()) {
			// make it possible for consumer to retry later if provider is temporarily
			// unavailable
			initialized = false;
