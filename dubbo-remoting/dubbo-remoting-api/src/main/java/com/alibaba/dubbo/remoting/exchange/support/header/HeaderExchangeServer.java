...
public class HeaderExchangeServer implements ExchangeServer {
	...
	public void reset(URL url) {
		...
		try {
			if (url.hasParameter(Constants.HEARTBEAT_KEY) || url.hasParameter(Constants.HEARTBEAT_TIMEOUT_KEY)) {
				int h = url.getParameter(Constants.HEARTBEAT_KEY, heartbeat);// 心跳间隔
				int t = url.getParameter(Constants.HEARTBEAT_TIMEOUT_KEY, h * 3);// 心跳超时时间
				if (t < h * 2) {
					throw new IllegalStateException("heartbeatTimeout < heartbeatInterval * 2");
				}
	...
	private void startHeartbeatTimer() {
		stopHeartbeatTimer();// 停止之前的心跳服务
		if (heartbeat > 0) {// 重新启动心跳服务
