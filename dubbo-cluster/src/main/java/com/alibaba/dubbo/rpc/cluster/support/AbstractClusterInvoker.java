...
public abstract class AbstractClusterInvoker<T> implements Invoker<T> {
	 ...
	 * b)Reslection, the validation rule for reselection: selected > available. This
	 * rule guarantees that the selected invoker has the minimum chance to be one in
	 * the previously selected list, and also guarantees this invoker is available.
	 *
	...
	public Result invoke(final Invocation invocation) throws RpcException {
		...
		List<Invoker<T>> invokers = list(invocation);// 得到所有的invokers（即服务提供者）

		if (invokers != null && !invokers.isEmpty()) {// 选择负载均衡器
			loadbalance = ExtensionLoader.getExtensionLoader(LoadBalance.class)
